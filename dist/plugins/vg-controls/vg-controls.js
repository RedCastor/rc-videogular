            ...
            if (API.isConfig) {
                scope.$watch("API.config", function() {
                    if (scope.API.config) {
                        ...
                        var ahtValue = scope.API.config.plugins.controls.autohideTime || 2e3;
            ...
            scope.updateCuePoints = function onUpdateCuePoints(cuePoints) {
                ...
                if (cuePoints) {
                    ...
                    for (var i = 0, l = cuePoints.length; i < l; i++) {
                        ...
                        var cuePointDuration = (end - cuePoints[i].timeLapse.start) * 1e3;
                        var position = cuePoints[i].timeLapse.start * 100 / Math.round(API.totalTime / 1e3) + "%";
                        var percentWidth = 0;
    ...
    return {
        ...
        link: function(scope, elem, attr, API) {
            ...
            scope.updateThumbnails = function(second) {
                var percentage = Math.round(second * 100 / (API.totalTime / 1e3));
                ...
                if (isStrip) {
                    ...
                    var secondsByPixel = API.totalTime / slider.scrollWidth / 1e3;
                    var lapse = {
...
angular.module("com.2fdevs.videogular.plugins.controls").run([ "$templateCache", function($templateCache) {
    $templateCache.put("vg-templates/vg-volume-bar", '<div class="verticalVolumeBar">' + '<div class="volumeBackground" ng-click="onClickVolume($event)" ng-mousedown="onMouseDownVolume()" ng-mouseup="onMouseUpVolume()" ng-mousemove="onMouseMoveVolume($event)" ng-mouseleave="onMouseLeaveVolume()">' + '<div class="volumeValue">' + "</div>" + '<div class="volumeClickArea">' + "</div></div></div>");
